...
export const CONTACT_RECOVERY_MIN_SECONDS = .28;
export const CONTACT_RECOVERY_MAX_SECONDS = .55;
...
export function createContactRecovery(start, target) {
  ...
  return {
    ...
    target: { ...target },
    elapsed: 0,
    duration: clamp(.24 + distance * .12, CONTACT_RECOVERY_MIN_SECONDS, CONTACT_RECOVERY_MAX_SECONDS)
  };
}
...
export function sampleContactRecovery(recovery, elapsed = recovery.elapsed) {
  ...
  const hop = Math.sin(progress * Math.PI) * .02;
  return {
