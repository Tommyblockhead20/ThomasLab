...
V4.1 keeps that functionality while reducing visual overload. Project recovery/import/export commands live in one header menu; alternate views and diagnostic overlays have compact viewport menus; Object/Mesh/Sculpt/Water/Path reveal only relevant tools; large outliner groups default closed and row actions reveal on hover or selection. `Ctrl/Cmd+Shift+Z` now performs Redo (alongside `Ctrl/Cmd+Y`) instead of accidentally issuing another Undo. Production-island adapters include the exact active shore benches, Aquarium visitor benches, and Mangrove fishing log as separately editable parts with runtime-facing metadata.
...
V4.1 also exposes every physical satellite island: Hearthward, Outfitter's Reach, Glasswater, Mangrove Cay, and Frosthook have dedicated exact production-terrain views; Skyreach Foundation terrain appears in the Skyscraper view; and Basalt shows the exact generated outer terrain whenever the higher-detail full cave capture is unavailable. Pirate and Library retain specialized scenes. Bluewater Reach is a registry-defined virtual boat/open-water location and therefore has no island terrain tab. Satellite edits autosave/export independently and are not silently promoted into the runtime generator. Hearthward, Outfitter's Reach, and Glasswater carry direct, individually selectable production-reference parts for their full cabin/shop/aquarium compositions, benches, local landscaping and current docks; the shop includes both service NPCs. Production and trail signs expose editable text in the inspector. An editor-only ocean sheet uses the live production waterline for shoreline context but is never exported as authored gameplay water. The complete asset catalog, including `Old Man Fisher NPC`, is searchable on every generic-world tab; `Place Asset` waits for the next terrain/surface click and aligns the asset's bottom bounds to that point.
