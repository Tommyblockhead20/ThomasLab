...
import { effectiveSmoothStrength, GenericWorldScene, sculptTriangleMesh } from './generic-scene.js';
...
import { recordActionSnapshot, redoActionSnapshot, undoActionSnapshot } from './action-history.js';
import {
  boundaryLoops,
  bridgeEdgeChains,
  createFace,
...
let activeWorldId = (() => {
  try {
    const linkedWorld = new URLSearchParams(location.search).get('world');
    if (WORLD_EDITOR_WORLDS.some((world) => world.id === linkedWorld)) return linkedWorld;
...
let zFightResults = [];
const ignoredZFightPairs = new Set();
...
window.addEventListener('reel-ascent:editor-status', (event) => {
  const detail = event.detail || {};
  if (detail.worldId && detail.worldId !== activeWorldId) return;
  if (detail.message) setStatus(detail.message);
  if (detail.state === 'ready' && detail.worldId === 'skyscraper' && detail.bounds) {
    const { center, halfExtents } = detail.bounds;
    cameraState.target.set(center.x, center.y, center.z);
    cameraState.distance = Math.max(60, Math.hypot(halfExtents.x, halfExtents.y, halfExtents.z) * 2.6);
    cameraState.yaw = 38;
    cameraState.pitch = -10;
    updateCamera();
  }
});
...
function createPlacedEntity(item, isSelected = false) {
  ...
  if (item.type === 'rock') {
    ...
    const signSurface = buildDecorPreview(root, item.decorKind, material);
    if (signSurface && supportsEditableSignText(item)) attachEditorSignText(app.graphicsDevice, signSurface, item);
  }
  return root;
}
...
function buildDecorPreview(root, kind) {
  if (kind === 'bench') {
    ...
    return addPrimitive(root, 'box', materials.decor, [0, 1.55, 0], [1.25, .5, .1]);
...
function releaseEditorOwnedMeshes(entity, seen = new Set()) {
  ...
  for (const material of entity._editorOwnedSignMaterials ?? []) { try { material.destroy(); } catch {} }
  for (const texture of entity._editorOwnedSignTextures ?? []) { try { texture.destroy(); } catch {} }
  entity._editorOwnedSignMaterials = null;
  entity._editorOwnedSignTextures = null;
}
...
function ensurePirateStarterComposition(level) {
  ...
  if (level.metadata?.pirateStarterCompositionV4) return level;
  const placements = [
    ['dock-section', { x: 20, y: .18, z: 2 }, 88],
    ['ship-bow', { x: -18, y: .5, z: 9 }, -28],
    ['ship-mid', { x: -12.5, y: .65, z: 6 }, -28],
    ['mast-rig', { x: -10, y: 1.1, z: 5 }, -18],
    ['treasure-camp', { x: 5, y: 2.1, z: -4 }, 15],
    ['palm-cluster', { x: 10, y: 1.4, z: 8 }, 12],
    ['reef-rocks', { x: 18, y: -.25, z: -10 }, 33],
    ['pirate-watch-scaffold-v3', { x: -2, y: 2.1, z: 7 }, -10]
  ];
  const v3Assets = new Map(assetsForWorld('pirate-island').map((asset) => [asset.id, asset]));
  for (const [assetId, position, yaw] of placements) {
    let definition;
    const pirateAsset = PIRATE_ASSET_LIBRARY.find((asset) => asset.id === assetId);
    if (pirateAsset) definition = makePirateAssetDefinition(assetId);
    else {
      const asset = v3Assets.get(assetId);
      if (!asset) continue;
      definition = {
        id: `EDITOR-ASSET-${asset.id.toUpperCase()}`, name: asset.label, kind: 'assembly', version: 1,
        objects: structuredClone(asset.objects), movingPlatforms: [], waters: [],
        metadata: { editorAssetId: asset.id, description: asset.description }
      };
    }
    if (!level.prefabs.definitions.some((item) => item.id === definition.id)) level.prefabs.definitions.push(definition);
    const id = `PIRATE-STARTER-${assetId.toUpperCase()}`;
    if (!level.prefabs.instances.some((item) => item.id === id)) level.prefabs.instances.push({
      id, name: definition.name, prefabId: definition.id, linked: true,
      transform: { position: { ...position }, rotation: { x: 0, y: yaw, z: 0 }, scale: { x: 1, y: 1, z: 1 } },
      metadata: { pirateStarterV4: true, assetId }
    });
  }
  level.metadata = { ...level.metadata, pirateStarterCompositionV4: true };
  return level;
}
...
async function loadGenericProjectLevel(world, { preferAutosave = true } = {}) {
  ...
  const level = normalizeWorldEditorLevel(payload, {
...
function saveGenericHistory() {
  ...
  recordActionSnapshot(stack, genericFutureFor(), JSON.stringify(genericStoredPayload(level)));
}
...
async function switchWorld(worldId, { keepCamera = false } = {}) {
  ...
  if (assetPlacementState) cancelV3AssetPlacement();
...
function undo() {
  if (!isStoneveilWorld()) {
    const stack = genericHistoryFor();
    const futureStack = genericFutureFor();
    const previous = undoActionSnapshot(stack, futureStack, JSON.stringify(genericStoredPayload()));
    if (previous === null) return;
    replaceActiveGenericLevel(JSON.parse(previous), { persist: true });
    rebuildAll();
    return;
  }
...
function redo() {
  if (!isStoneveilWorld()) {
    ...
    const historyStack = genericHistoryFor();
    const next = redoActionSnapshot(historyStack, stack, JSON.stringify(genericStoredPayload()));
    if (next === null) return;
    replaceActiveGenericLevel(JSON.parse(next), { persist: true });
    rebuildAll();
    return;
  }
...
function smoothBakedMesh(hit, radius, strength) {
  ...
  const baseBlend = clamp(effectiveSmoothStrength(strength) * .08, .02, .55);
  let changed = 0;
  for (const [index, distance, localRadius] of brush.affected) {
...
function updateMeshAuthoringUi() {
  ...
  const selectedVertexCount = selectedMeshVertexIds(mesh).length;
  const enabled = (id, value, reason) => {
    const button = $(id); if (!button) return;
    button.disabled = !value; button.title = value ? '' : reason;
  };
  enabled('#mesh-apply-move', selectedVertexCount > 0, 'Select vertices, edges, or faces first.');
  ...
  enabled('#mesh-inflate', selectedVertexCount > 0, 'Select mesh elements first.');
  enabled('#mesh-deflate', selectedVertexCount > 0, 'Select mesh elements first.');
  enabled('#mesh-subdivide', meshSelection.faces.size > 0, 'Select one or more faces.');
  enabled('#mesh-weld', selectedVertexCount >= 2, 'Select at least two vertices.');
...
function setAuthoringMode(mode = 'object') {
  ...
  const availableModes = {
    object: worldHasCapability(world.id, 'objects') || worldHasCapability(world.id, 'rocks') || worldHasCapability(world.id, 'decor') || worldHasCapability(world.id, 'parkour'),
    mesh: isStoneveilWorld() ? isMeshMode() : Boolean(genericScene?.editableTerrainMesh?.()),
    sculpt: worldHasCapability(world.id, 'terrain'),
    water: worldHasCapability(world.id, 'water'),
    path: worldHasCapability(world.id, 'water')
  };
  for (const button of $$('[data-authoring-mode]')) button.hidden = !availableModes[button.dataset.authoringMode];
  const requested = ['object', 'mesh', 'sculpt', 'water', 'path'].includes(mode) ? mode : 'object';
  authoringMode = availableModes[requested] ? requested : (Object.keys(availableModes).find((key) => availableModes[key]) || 'object');
  ...
  if ($('#library-tools-section')) $('#library-tools-section').hidden = isStoneveilWorld() || !['object', 'mesh'].includes(authoringMode);
  if ($('#brush-section')) $('#brush-section').hidden = authoringMode !== 'sculpt' || !worldHasCapability(world.id, 'terrain');
  if ($('#mesh-mode-section')) $('#mesh-mode-section').hidden = !isStoneveilWorld() || !['mesh', 'sculpt'].includes(authoringMode);
  for (const id of ['rock-palette-section', 'plant-palette-section', 'decor-palette-section']) {
    const element = $(`#${id}`);
    if (element) element.hidden = !isStoneveilWorld() || authoringMode !== 'object';
  }
  if ($('#generic-palette-section')) $('#generic-palette-section').hidden = authoringMode !== 'object'
    || !(worldHasCapability(world.id, 'parkour') || worldHasCapability(world.id, 'objects') || worldHasCapability(world.id, 'prefabs') || worldHasCapability(world.id, 'rooms'));
  if ($('#cave-migration-section')) $('#cave-migration-section').hidden = world.id !== 'cave-fishing-island' || !['mesh', 'sculpt'].includes(authoringMode);
  if ($('#pirate-asset-library-section')) $('#pirate-asset-library-section').hidden = world.id !== 'pirate-island' || authoringMode !== 'object';
  if ($('#skyscraper-room-library-section')) $('#skyscraper-room-library-section').hidden = world.id !== 'skyscraper' || authoringMode !== 'object';
  const toolModes = {
    select: ['object', 'mesh', 'sculpt', 'water', 'path'],
    raise: ['sculpt'], lower: ['sculpt'], smooth: ['sculpt'], indent: ['sculpt'], 'pull-core': ['sculpt'], 'connect-cave': ['sculpt'],
    rock: ['object'], plant: ['object'], decor: ['object'], object: ['object'], platform: ['object'], 'moving-platform': ['object'],
    'move-water': ['water']
  };
  ...
  for (const button of $$('#tool-grid button')) {
    ...
    const supported = !capability || worldHasCapability(world.id, capability);
    button.hidden = !supported || !(toolModes[button.dataset.tool] ?? ['object']).includes(authoringMode);
  }
  const modeCopy = {
    object: ['Object Tools', 'Select, place, and edit complete scene objects. Asset, prefab, and structure controls are grouped below.'],
    mesh: ['Mesh Tools', 'Select terrain vertices, edges, and faces. Topology and opening tools are shown below.'],
    sculpt: ['Terrain Sculpt', 'Choose one brush, then edit terrain in the viewport. Each stroke is one undoable step.'],
    water: ['Water Tools', 'Select water volumes or use Move Water. Path-node editing has its own mode.'],
    path: ['Water Path', 'Select a path water, then use the compact path controls below.']
  }[authoringMode];
  if ($('#tool-section-heading')) $('#tool-section-heading').textContent = modeCopy[0];
  if ($('#authoring-mode-help')) $('#authoring-mode-help').textContent = modeCopy[1];
  if (authoringMode === 'sculpt' && !['raise', 'lower', 'smooth', 'indent', 'pull-core'].includes(tool)) tool = 'raise';
  if (['mesh', 'path'].includes(authoringMode)) tool = 'select';
  if (authoringMode === 'water' && tool !== 'move-water') tool = 'select';
  if (authoringMode === 'object' && toolModes[tool] && !toolModes[tool].includes('object')) tool = 'select';
...
function selectBasaltMeshElement(event, ray) {
  ...
  selected = { kind: 'terrain-mesh', id: genericScene.terrainMeshId(), record: mesh };
...
function mutateBasaltMesh(operation, successMessage) {
  ...
  try {
    commitGeneric((draft) => {
      ...
      draft.terrain.mode = originalMode;
      draft.sourcePolicy.terrain = originalMode === 'authored-mesh-candidate' ? 'authored-candidate' : 'authored';
    });
    updateMeshAuthoringUi();
    setStatus(originalMode === 'authored-mesh-candidate'
      ? `${successMessage} The candidate remains non-production until explicitly promoted.`
      : `${successMessage} Pirate Island terrain remains an editable authored mesh.`);
  } catch (error) {
...
function applyGenericToolAt(event) {
  ...
  if (genericScene.editableTerrainMesh() && ['raise', 'lower', 'smooth', 'indent', 'pull-core'].includes(tool)) {
    ...
    }, { recordHistory: !activeBrushTransaction });
    if (changed) {
      if (activeWorldId === 'cave-fishing-island') {
        genericScene.setBasaltReferenceMode('authored');
        if ($('#basalt-reference-mode')) $('#basalt-reference-mode').value = 'authored';
      }
      setStatus(activeWorldId === 'cave-fishing-island'
        ? `Cave Island ${tool}: edited ${changed} connected terrain vertices.`
        : `${activeWorld().label} ${tool}: edited ${changed} connected terrain vertices.`);
    }
    return;
  }
  if (tool === 'move-water') {
...
function applyToolAt(event, continuous = false) {
  ...
  if (tool === 'decor') {
    const item = {
      ...
      position: { x: hit.x, y: hit.y, z: hit.z }, rotation: { x: 0, y: Math.random() * 360, z: 0 },
      scale: { x: 1, y: 1, z: 1 },
      ...($('#decor-kind').value === 'sign' ? { metadata: { editableSign: true, signText: 'TRAIL' } } : {})
    };
    commit(() => patch.placedObjects.push(item), { placed: true });
  }
}
...
function syncWorldUi() {
  ...
  if ($('#library-tools-section')) $('#library-tools-section').hidden = stoneveil;
...
function renderGenericSelection() {
  ...
  if (supportsEditableSignText(record)) appendEditableSignTextField(fields, record, (value) => {
    mutateSelectedGenericRecord((item) => {
      item.metadata ??= {};
      item.metadata.editableSign = true;
      item.metadata.signText = value;
    });
  });
...
function renderOutliner() {
  ...
  if (isStoneveilWorld()) {
    ...
    if (level?.terrain?.positions?.length && level?.terrain?.indices?.length) {
      ...
      groups.push(['Terrain Mesh Parts', (level.terrain.parts ?? []).map((part, index) => ({
        id:`__terrain-part__:${index}`, name:part.name || `Mesh part ${index + 1}`, kind:'terrain-mesh-part', type:`${part.triangleCount || 0} faces`,
    ...
    } else if (level?.metadata?.productionIslandAdapter) {
      const sourceObjects = level?.objects ?? [];
      const categories = [...new Set(sourceObjects.map((item) => item.category || 'Objects'))].sort((a, b) => a.localeCompare(b));
      for (const category of categories) {
        const items = sourceObjects.filter((item) => (item.category || 'Objects') === category).map((item) => ({
          id:item.id, name:item.name||item.id, kind:'world-object', type:item.type||'object',
          renameable:true, deletable:true, hideable:true
        }));
        if (items.length) groups.push([category, items]);
  ...
  for (const [name, sourceItems] of groups) {
    ...
    const collapsed = Object.hasOwn(prefs, groupKey) ? Boolean(prefs[groupKey]) : items.length > 20;
...
function renderSelection() {
  ...
  if (selected.kind === 'water') {
    ...
    $('#hide-selected').disabled = false;
    if (selected.kind === 'placed' && supportsEditableSignText(record)) appendEditableSignTextField(fields, record, (value) => {
      commit(() => {
...
function appendEditableSignTextField(parent, record, onCommit) {
  const wrapper = document.createElement('label');
  wrapper.className = 'inspector-name';
  wrapper.textContent = 'Sign text';
  const input = document.createElement('input');
  input.type = 'text';
  input.maxLength = 48;
  input.value = editableSignText(record);
  input.addEventListener('change', () => onCommit(String(input.value || '').trim().slice(0, 48) || 'SIGN'));
...
function setupCollapsibleSections() {
  ...
  const initiallyCollapsed = new Set(['summary-section', 'validation-section', 'geometry-diagnostics-section', 'camera-section']);
  ...
  sections.forEach((section, index) => {
    ...
    const collapsed = Object.hasOwn(prefs, key) ? Boolean(prefs[key]) : initiallyCollapsed.has(section.id);
...
function populateV3AssetUi() {
  ...
  if (!select) return;
  const query = String($('#v3-asset-search')?.value || '').trim().toLowerCase();
  const assets = assetsForWorld(activeWorldId).filter((asset) => !query
    || `${asset.name} ${asset.category} ${asset.description}`.toLowerCase().includes(query));
  ...
  for (const [category, categoryAssets] of [...byCategory].sort(([a], [b]) => a.localeCompare(b))) {
    const group = document.createElement('optgroup'); group.label = category;
    for (const asset of categoryAssets.sort((a, b) => a.name.localeCompare(b.name))) group.append(new Option(asset.name, asset.id));
  ...
  syncV3AssetPlacementUi();
}

function syncV3AssetPlacementUi() {
  const button = $('#place-v3-asset');
  if (!button) return;
  const armed = Boolean(assetPlacementState && assetPlacementState.worldId === activeWorldId);
  button.classList.toggle('active', armed);
  button.textContent = armed ? 'Cancel Asset Placement' : 'Place Asset';
}

function cancelV3AssetPlacement(message = '') {
  assetPlacementState = null;
  syncV3AssetPlacementUi();
  if (message) setStatus(message);
}

function assetDefinitionFloor(definition) {
  let floor = Infinity;
  for (const item of definition?.objects ?? []) {
    const y = Number(item.transform?.position?.y) || 0;
    const halfHeight = Math.abs(Number(item.size?.y) || 0) * .5;
    floor = Math.min(floor, y - halfHeight);
  }
...
function placeV3Asset() {
  ...
  if (!level || !asset) return;
  if (assetPlacementState?.worldId === activeWorldId && assetPlacementState.assetId === asset.id) {
    cancelV3AssetPlacement('Asset placement cancelled.');
    return;
  }
  assetPlacementState = { assetId: asset.id, worldId: activeWorldId };
  syncV3AssetPlacementUi();
  setStatus(`${asset.name} ready. Click the terrain or another placement surface to put it there; Esc cancels.`);
}

function commitV3AssetPlacement(surface) {
  const placement = assetPlacementState;
  if (!placement || placement.worldId !== activeWorldId) return false;
  const level = activeGenericLevel();
  const asset = getV3Asset(placement.assetId);
  if (!level || !asset || !surface) return false;
  ...
  const clicked = maybeSnapPosition({ x: surface.x, y: surface.y, z: surface.z });
  const position = { ...clicked, y: clicked.y - assetDefinitionFloor(definition) };
  const instance = {
  ...
  assetPlacementState = null;
  renderUi();
  setStatus(`Placed ${asset.name} at the clicked surface. It is selected and listed under Prefab Instances; use Edit Source Prefab to edit its individual parts.`);
  return true;
}
...
function zFightSourceRecords(level = activeGenericLevel()) {
  if (!level) return [];
  if (prefabWorkspace.active) return activePrefabDefinition(level)?.objects ?? [];
  return level.objects ?? [];
}

function zFightPairKey(result) {
  return [String(result.objectId), String(result.otherObjectId)].sort().join('::');
}

function scanZFighting() {
  if (isStoneveilWorld()) {
    zFightResults = [];
    renderZFightingResults('Stoneveil uses the dedicated terrain/cave diagnostics; this scan targets authored object geometry.');
    return;
  }
  const threshold = Math.max(.0001, Number($('#zfight-threshold')?.value) || .01);
  zFightResults = findLikelyZFighting(zFightSourceRecords(), threshold)
    .filter((result) => !ignoredZFightPairs.has(zFightPairKey(result)));
  renderZFightingResults();
  setStatus(zFightResults.length
    ? `Found ${zFightResults.length} likely z-fighting pair(s). Cyan marks A; magenta marks B.`
    : 'No likely authored-object z-fighting pairs found at this threshold.');
}

function focusZFightPair(result, selectSecond = false) {
  const id = String(selectSecond ? result.otherObjectId : result.objectId);
  genericScene.setDiagnosticPair(result.objectId, result.otherObjectId);
  selected = { kind: prefabWorkspace.active ? 'prefab-child-object' : 'world-object', id };
  genericScene.setSelected(id);
  focusSelectedObject();
  genericScene.setDiagnosticPair(result.objectId, result.otherObjectId);
  renderUi();
...
function mutateZFightObject(id, mutator) {
  commitGeneric((draft) => {
    const source = prefabWorkspace.active
      ? (draft.prefabs?.definitions ?? []).find((item) => item.id === prefabWorkspace.definitionId)?.objects
      : draft.objects;
    const record = (source ?? []).find((item) => item.id === id);
    if (record) mutator(record, source);
  });
  scanZFighting();
}

function renderZFightingResults(emptyMessage = '') {
  const container = $('#zfight-results');
  if (!container) return;
  container.replaceChildren();
  if (!zFightResults.length) {
    container.textContent = emptyMessage || 'No likely overlaps in the current scan.';
    return;
  }
  const threshold = Math.max(.0001, Number($('#zfight-threshold')?.value) || .01);
  for (const result of zFightResults) {
    const row = document.createElement('div');
    row.className = 'diagnostic-result';
    const title = document.createElement('strong');
    title.textContent = result.code === 'duplicate-overlapping-surface' ? 'Exact overlapping bounds' : `Near-coplanar ${String(result.axis).toUpperCase()} surfaces`;
    row.append(
      title,
      Object.assign(document.createElement('div'), { textContent: `A: ${result.objectId} · B: ${result.otherObjectId} · separation ${Number(result.separation || 0).toFixed(4)} m` }),
      Object.assign(document.createElement('div'), { textContent: result.message })
    );
    const actions = document.createElement('div'); actions.className = 'row-actions';
    const button = (label, handler, disabled = false, titleText = '') => {
      const element = document.createElement('button'); element.type = 'button'; element.textContent = label; element.disabled = disabled; element.title = titleText;
      if (!disabled) element.addEventListener('click', handler);
      actions.appendChild(element);
    };
    button('Focus pair', () => focusZFightPair(result));
    button('Select A', () => focusZFightPair(result, false));
    button('Select B', () => focusZFightPair(result, true));
    if (result.axis) {
      button(`Offset A +${result.axis.toUpperCase()}`, () => mutateZFightObject(result.objectId, (record) => { record.transform.position[result.axis] += Math.max(.002, threshold * 2); }));
      button(`Offset A −${result.axis.toUpperCase()}`, () => mutateZFightObject(result.objectId, (record) => { record.transform.position[result.axis] -= Math.max(.002, threshold * 2); }));
      button(`Offset B +${result.axis.toUpperCase()}`, () => mutateZFightObject(result.otherObjectId, (record) => { record.transform.position[result.axis] += Math.max(.002, threshold * 2); }));
      button(`Offset B −${result.axis.toUpperCase()}`, () => mutateZFightObject(result.otherObjectId, (record) => { record.transform.position[result.axis] -= Math.max(.002, threshold * 2); }));
    }
    const exact = result.code === 'duplicate-overlapping-surface';
    button('Merge Coplanar', () => {
      if (!confirm(`Remove duplicate ${result.otherObjectId} and keep ${result.objectId}?`)) return;
      mutateZFightObject(result.otherObjectId, (_record, source) => source.splice(source.findIndex((item) => item.id === result.otherObjectId), 1));
    }, !exact, exact ? '' : 'Safe merge is only offered for identical bounds.');
    button(exact ? 'Delete Exact Duplicate' : 'Delete B', () => {
      if (!confirm(`Delete ${result.otherObjectId}?`)) return;
      mutateZFightObject(result.otherObjectId, (_record, source) => source.splice(source.findIndex((item) => item.id === result.otherObjectId), 1));
    });
    button('Ignore pair', () => { ignoredZFightPairs.add(zFightPairKey(result)); scanZFighting(); });
...
function setupV21Ui() {
  ...
  $('#scan-zfighting')?.addEventListener('click', scanZFighting);
  ...
  $('#v3-asset-template')?.addEventListener('change', () => { if (assetPlacementState) cancelV3AssetPlacement(); populateV3AssetUi(); });
  ...
  $('#show-picking-targets')?.addEventListener('change', (event) => {
...
function selectedEdgeChains(edges) {
  ...
  while (remaining.size) {
    ...
    while (queue.length) {
      const id = queue.pop(); if (component.has(id)) continue;
      component.add(id); remaining.delete(id);
      for (const next of adjacency.get(id) ?? []) if (!component.has(next)) queue.push(next);
    }
    const endpoints = [...component].filter((id) => (adjacency.get(id)?.size || 0) === 1);
    if (endpoints.length !== 2) return [];
    const chain = [endpoints[0]];
    let previous = null, current = endpoints[0];
    while (current !== endpoints[1]) {
      const next = [...(adjacency.get(current) ?? [])].find((id) => id !== previous);
      if (next == null) return [];
      chain.push(next); previous = current; current = next;
    }
    chains.push(chain);
  }
  return chains;
}
$('#mesh-bridge')?.addEventListener('click', () => {
  const chains = selectedEdgeChains(meshSelection.edges);
  if (chains.length !== 2 || chains[0].length !== chains[1].length) return setStatus('Bridge requires two separate connected edge chains with equal vertex counts.');
  mutateBasaltMesh((mesh) => bridgeEdgeChains(mesh, chains[0], chains[1]), `Bridged two ${chains[0].length}-vertex edge chains.`);
});
$('#mesh-flatten')?.addEventListener('click', () => mutateBasaltMesh(
  (mesh) => flattenSelection(mesh, meshSelection, $('#mesh-flatten-axis')?.value || 'y'), 'Flattened the selected topology.'
));
$('#mesh-inflate')?.addEventListener('click', () => mutateBasaltMesh(
  (mesh) => inflateSelection(mesh, meshSelection, Math.abs(Number($('#mesh-inflate-amount')?.value) || .15)), 'Inflated the selected topology along vertex normals.'
));
$('#mesh-deflate')?.addEventListener('click', () => mutateBasaltMesh(
  (mesh) => inflateSelection(mesh, meshSelection, -Math.abs(Number($('#mesh-inflate-amount')?.value) || .15)), 'Deflated the selected topology along vertex normals.'
));
$('#mesh-subdivide')?.addEventListener('click', () => {
  if (!meshSelection.faces.size) return setStatus('Select one or more faces to subdivide.');
  mutateBasaltMesh((mesh) => subdivideFaces(mesh, meshSelection.faces), 'Subdivided each selected face into four editable faces.');
  meshSelection = makeMeshSelection(); updateMeshAuthoringUi();
});
...
canvas.addEventListener('pointerdown', (event) => {
  ...
  if (!isStoneveilWorld() && assetPlacementState) {
    const surface = genericScene.surfaceHit(pointerRay(event));
    if (!surface) {
      setStatus('No terrain or placement surface is under the cursor. Asset placement is still active; click a visible surface or press Esc.');
      return;
    }
    commitV3AssetPlacement(surface);
    return;
  }
  ...
  const genericMeshSculpt = Boolean(!isStoneveilWorld() && genericScene.editableTerrainMesh())
  ...
  const stoneveilSculpt = isStoneveilWorld()
    && ['raise', 'lower', 'smooth', 'indent', 'pull-core', 'plant'].includes(tool);
  draggingBrush = genericMeshSculpt || stoneveilSculpt;
  activeBrushTransaction = draggingBrush;
  if (activeBrushTransaction) {
    if (isStoneveilWorld()) saveHistory(); else saveGenericHistory();
  }
  lastBrushPoint = null;
...
window.addEventListener('keydown', (event) => {
  ...
  if (historyCommand) {
    event.preventDefault();
    if (historyCommand === 'redo') redo(); else undo();
    return;
  }
  ...
  if (event.key === 'Escape' && assetPlacementState) {
    cancelV3AssetPlacement('Asset placement cancelled.');
    event.preventDefault();
    return;
  }
